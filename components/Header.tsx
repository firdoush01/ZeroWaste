...
export default function Header({ onMenuClick, totalEarnings }: HeaderProps) {
  const { data: session, status } = useSession();  // `useSession` returns loading status
  ...
  if (status === "loading") {
    return <div>Loading...</div>; // Show loading state while session is being determined
  }
    ...
    <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
      <div className="flex items-center justify-between px-4 py-2">
        ...
        <div className="flex items-center">
          ...
          {!session ? (
            <Button onClick={() => signIn()} className="bg-green-600 hover:bg-green-700 text-white text-sm md:text-base">
              Login
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem>{session.user.name || "Anonymous User"}</DropdownMenuItem>
                <DropdownMenuItem>
